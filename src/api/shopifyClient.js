...
async function getCollectionProducts(shop, token, colId) {
  ...
  const q = `query($id:ID!,$c:String){collection(id:$id){products(first:250,after:$c){pageInfo{hasNextPage endCursor}edges{node{id title status featuredImage{id} productType tags kategorija:metafield(namespace:"custom",key:"kategorija"){value} kategorija1:metafield(namespace:"custom",key:"kategorija1"){value} variants(first:100){edges{node{id price inventoryQuantity selectedOptions{name value}}}}}}}}}`;
  const prods=[]; let cursor=null,more=true;
  while(more){const d=await gql(shop,token,q,{id:gid,c:cursor});const col=d?.collection;if(!col)break;for(const e of col.products.edges){const p=e.node;if(p.status!=="ACTIVE")continue;if(!p.featuredImage)continue;prods.push(norm(p));}more=col.products.pageInfo.hasNextPage;cursor=col.products.pageInfo.endCursor;if(more)await s(300);}
  return prods;
}
function norm(p){const id=p.id.replace("gid://shopify/Product/","");const variants=p.variants.edges.map(e=>({id:e.node.id.replace("gid://shopify/ProductVariant/",""),price:e.node.price,inventory_quantity:e.node.inventoryQuantity,options:e.node.selectedOptions}));const co=variants[0]?.options?.find(o=>["color","colour","boja","farba"].includes(o.name.toLowerCase()));const kategorija=(p.kategorija?.value||p.kategorija1?.value||"").trim();return{id,title:p.title,product_type:p.productType,tags:p.tags?.join(",")||"",color:co?.value||"",kategorija,variants};}
