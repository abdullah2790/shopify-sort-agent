...
async function getCollections(shop, token, queryFilter = "") {
  ...
  const q=`query($c:String){collections(first:250,after:$c${qArg}){pageInfo{hasNextPage endCursor}edges{node{id title handle onlineStoreUrl productsCount{count}}}}}`;
  const cols=[]; let cursor=null,more=true;
  while(more){const d=await gql(shop,token,q,{c:cursor});for(const e of d.collections.edges){const c=e.node;cols.push({id:c.id.replace("gid://shopify/Collection/",""),title:c.title,handle:c.handle,onlineStoreUrl:c.onlineStoreUrl??null,productsCount:c.productsCount?.count??0});}more=d.collections.pageInfo.hasNextPage;cursor=d.collections.pageInfo.endCursor;if(more)await s(300);}
  return cols;
}
